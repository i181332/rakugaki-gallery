  ...
  return (
    ...
    <div className="w-full max-w-md flex flex-col items-center justify-center gap-8 transition-all duration-300">
      ...
      <div className="text-center">
        {/* UI.html: class="text-4xl font-extrabold text-slate-800 tracking-tight" */}
        <h1 className="text-4xl font-extrabold text-slate-800 tracking-tight">
          <span className="text-indigo-500">Rakugaki</span> Gallery
        </h1>
        {/* UI.html: class="text-slate-500 text-sm mt-2 font-bold" */}
      ...
      {/* メインカード */}
      {/* UI.html: class="bg-white p-8 rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100 w-full" */}
      <div className="bg-white p-8 rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100 w-full">
        <div className="flex flex-col items-center gap-6">
          {/* アバター */}
          {/* UI.html: class="relative group cursor-pointer" */}
          <div className="relative group cursor-pointer" onClick={handleRandomAvatar}>
            {/* UI.html: class="w-24 h-24 bg-indigo-100 rounded-full border-4 border-white shadow-md flex items-center justify-center overflow-hidden" */}
            ...
            {/* UI.html: class="absolute bottom-0 right-0 bg-white rounded-full p-1.5 shadow-sm border border-slate-100 text-slate-400 hover:text-indigo-500 transition" */}
          ...
          <div className="w-full">
            {/* UI.html: class="block text-xs font-bold text-slate-400 mb-2 ml-2" */}
            <label
              htmlFor="nickname"
            ...
            <div className="relative">
              {/* UI.html: class="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3 px-4 pl-10 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition" */}
              <input
                type="text"
                id="nickname"
                placeholder="名前を入力..."
                ...
                maxLength={20}
              />
              {/* UI.html: class="fas fa-palette absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-400" */}
              <svg
          ...
          {/* スタートボタン */}
          {/* UI.html: class="w-full bg-slate-800 hover:bg-indigo-600 text-white rounded-2xl py-4 font-extrabold text-lg shadow-lg shadow-indigo-200 transition transform hover:-translate-y-1 flex items-center justify-center gap-2" */}
          <button
            onClick={handleStart}
