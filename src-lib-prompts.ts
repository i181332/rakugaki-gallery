...
以下の要素を総合的に評価し、100万円〜10億円の範囲で価格を決定せよ。
...

{
  ...
  "price": 数値のみ（100万〜10億の整数。カンマや単位は含めない）,
...

{
  ...
  "price": 数値のみ（前作価格を基準に増減。100万〜10億の範囲内）,
...
export function buildContinuationPrompt(previousWork: PreviousWorkInfo): string {
  return CONTINUATION_CRITIQUE_TEMPLATE
    ...
    .replace('{{previousArtist}}', previousWork.artist)
    .replace(/\{\{previousArtist\}\}/g, previousWork.artist) // 複数箇所置換
